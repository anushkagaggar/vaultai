...
export default function BudgetPlanPage() {
  ...
  const scenarios = [plan.scenarios.base, plan.scenarios.optimistic, plan.scenarios.conservative];
  ...
  return (
    <AuthenticatedLayout title="Budget Plan">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ background: '#1A1D27', border: '1px solid #2E3248', borderRadius: 12, padding: 24 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <span style={{ padding: '5px 12px', borderRadius: 6, fontSize: 13, fontWeight: 600, background: 'rgba(59,130,246,0.15)', color: '#3B82F6' }}>
                Budget Plan
              </span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 24, alignItems: 'start' }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: '#CBD5E1', margin: 0 }}>
                Based on your income of {formatIndianCurrency(plan.incomeMonthly)}/month
              </p>
            </div>
        ...
        {/* Key Parameters */}
        <div style={{ background: '#1A1D27', border: '1px solid #2E3248', borderRadius: 12, padding: 24 }}>
          <p style={{ fontSize: 12, fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 16px' }}>
            Key Parameters
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20 }}>
            {[
              { label: 'Monthly Income',   value: formatIndianCurrency(plan.incomeMonthly) },
              { label: 'Inflation Rate',   value: `${(plan.inflationRate * 100).toFixed(1)}%` },
              { label: 'Min Savings Rate', value: `${(plan.minSavingsRate * 100).toFixed(0)}%` },
            ].map(({ label, value }) => (
              <div key={label}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          {[
            { label: 'Monthly Savings', value: plan.monthlySavings, sub: `${((plan.monthlySavings / plan.incomeMonthly) * 100).toFixed(1)}% of income`, color: '#10B981' },
            { label: 'Annual Savings',  value: plan.annualSavings,  sub: 'Projected for 12 months',                                                     color: '#6366F1' },
            ...
            <div key={label} style={{ background: '#1A1D27', border: '1px solid #2E3248', borderRadius: 12, padding: 24 }}>
              ...
              <p style={{ fontSize: 28, fontWeight: 800, color, margin: '0 0 4px', fontFamily: 'monospace' }}>
        ...
        <ProjectionChart data={plan.projection} label="12-Month Savings Projection" />
